...
let embed = new Discord.RichEmbed().setAuthor(message.member.displayName, message.author.avatarURL).setColor('#ff0000').setTimestamp().setThumbnail(message.author.avatarURL).setFooter('Tasia');
        ...
        db.push(`isimler_${member.id}`, ` \`${name} | ${age}\` (<@&807714811034992661>)`);
        ...
        db.add(`kız_${message.author.id}`, 1)
         ...
         message.guild.members.get(member.id).addRole(`807714811034992661`)
        message.channel.send(embed2.setDescription(`${member} adlı kullanıcı \`${name} | ${age}\` isminde kayıt edildi! (<@&807714811034992661>)`)
